...
def main():
    ...
    for monthCount in range(1,endMonth+1):
        iPaidPeriod = interestPaidThisPeriod(moPayment,annualIntRate,\
                                            mortgageAmt,monthCount)
        pPaidPeriod = principalPaidThisPeriod(moPayment,annualIntRate,\
                                            mortgageAmt,monthCount)
        totalInterest=totalInterestCalculator(moPayment,annualIntRate,\
                                            mortgageAmt,monthCount)
        newBalance = remainingBalance(moPayment,annualIntRate,\
                                            mortgageAmt,monthCount)
...
def interestPaidThisPeriod(moPayment,annualIntRate,mortgageAmt,monthCount):
    iCounter = 0
    totalInterest = 0
    effRate = annualIntRate / 12
    for iCounter in range(1,monthCount+1):
        iPaidPeriod = effRate*mortgageAmt
        pPaidPeriod = moPayment - iPaidPeriod
        totalInterest = totalInterest + iPaidPeriod
        mortgageAmt = mortgageAmt - pPaidPeriod
    return iPaidPeriod

#  Executing a loop for each loan month, up until the present month of the 
#  loop in main(), the principal paid in monthCount is calculated and returned
#  to the main function to continue processing.
def principalPaidThisPeriod(moPayment,annualIntRate,mortgageAmt,monthCount):
    iCounter = 0
    totalInterest = 0
    effRate = annualIntRate / 12
    for iCounter in range(1,monthCount+1):
...
#  Executing a loop for each loan month, up until the present month of the 
#  loop in main(), the total interest paid as of monthCount is calculated and
#  returned to the main function to continue processing.
def totalInterestCalculator(moPayment,annualIntRate,mortgageAmt,monthCount):
    iCounter = 0
    totalInterest = 0
    effRate = annualIntRate / 12
    for iCounter in range(1,monthCount+1):
...
#  Executing a loop for each loan month, up until the present month of the 
#  loop in main(), the remaining balance as of monthCount is calculated and
#  returned to the main function to continue processing.
def remainingBalance(moPayment,annualIntRate,mortgageAmt,monthCount):
    iCounter = 0
    totalInterest = 0
    effRate = annualIntRate / 12
    for iCounter in range(1,monthCount+1):
